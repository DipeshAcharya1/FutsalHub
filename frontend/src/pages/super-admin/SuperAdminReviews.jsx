...
const SuperAdminReviews = ({ futsals = [] }) => {
  ...
  useEffect(() => {
    fetchReviews();
  }, [currentPage, statusFilter, selectedFutsalId]);

  const fetchReviews = async () => {
    setLoading(true);
    try {
      let url = `/super-admin/reviews?page=${currentPage}&per_page=10&status=${statusFilter}`;
      if (selectedFutsalId) {
        url += `&futsal_id=${selectedFutsalId}`;
      }
  ...
  const handleApproveReview = async (reviewId) => {
    ...
    try {
      const response = await api.patch(`/super-admin/reviews/${reviewId}/moderate`, {
        is_approved: true
      });
      if (response.data.success) {
  ...
  const handleDeleteReview = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await api.delete(`/super-admin/reviews/${selectedReview.id}`);
      if (response.data.success) {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <span key={i} style={{ color: i <= numRating ? '#f39c12' : '#ddd', fontSize: '14px' }}>
          ★
        </span>
      );
    }
  ...
  return (
    <div>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#7f8c8d', margin: 0 }}>Manage all customer reviews across all futsals</p>
      </div>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {futsals.length > 0 && (
          <select 
            ...
            onChange={(e) => {
              setSelectedFutsalId(e.target.value);
              setCurrentPage(1);
            }}
            style={{ padding: '8px 16px', border: '1px solid #e0e0e0', borderRadius: '8px', marginLeft: 'auto' }}
          >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8f9fa', borderBottom: '2px solid #eef2f6' }}>
                <th style={{ padding: '16px', textAlign: 'left', fontWeight: '600', color: '#2c3e50' }}>Futsal</th>
            ...
            <tbody>
              {reviews.length > 0 ? (
                reviews.map((review) => (
                  <tr key={review.id} style={{ borderBottom: '1px solid #eef2f6' }}>
                    <td style={{ padding: '16px', verticalAlign: 'top' }}>
                      <div style={{ fontWeight: '600' }}>{review.futsal?.futsal_name || 'N/A'}</div>
                      <div style={{ fontSize: '12px', color: '#7f8c8d' }}>{review.futsal?.location || ''}</div>
                    </td>
                    <td style={{ padding: '16px', verticalAlign: 'top' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                        {!review.is_approved && (
                          <button 
                            ...
                            style={{ background: '#27ae60', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' }}
                          >
                            Approve
                          </button>
                        )}
                ...
                <tr>
                  <td colSpan="7" style={{ padding: '60px', textAlign: 'center', color: '#7f8c8d' }}>
                    No reviews found
                  </td>
                </tr>
              )}
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '24px', padding: '16px' }}>
          <button 
            ...
            style={{ padding: '8px 16px', border: '1px solid #e0e0e0', borderRadius: '6px', background: 'white', cursor: currentPage === 1 ? 'not-allowed' : 'pointer', opacity: currentPage === 1 ? 0.5 : 1 }}
          >
            Previous
          </button>
          <span style={{ padding: '8px 16px', color: '#2c3e50' }}>Page {currentPage} of {totalPages}</span>
          <button 
            onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} 
            disabled={currentPage === totalPages}
            style={{ padding: '8px 16px', border: '1px solid #e0e0e0', borderRadius: '6px', background: 'white', cursor: currentPage === totalPages ? 'not-allowed' : 'pointer', opacity: currentPage === totalPages ? 0.5 : 1 }}
          >
            Next
          </button>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: '20px' }}>
              ...
              <p><strong>Futsal:</strong> {selectedReview.futsal?.futsal_name}</p>
