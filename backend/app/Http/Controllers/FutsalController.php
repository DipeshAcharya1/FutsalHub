...
public function index(Request $request): JsonResponse
{
    try {
        ...
        $query = Futsal::query()->where('active', true);
        ...
        // Apply sorting - FIXED for price sorting
        switch ($sort) {
            case 'price_low':
                // Get futsals ordered by minimum slot price (lowest first)
                $query->select('futsals.*')
                      ->leftJoin('futsal_slots', 'futsals.id', '=', 'futsal_slots.futsal_id')
                      ->groupBy('futsals.id')
                      ->orderByRaw('MIN(futsal_slots.price) ASC');
                break;
                
            case 'price_high':
                // Get futsals ordered by minimum slot price (highest first)
                $query->select('futsals.*')
                      ->leftJoin('futsal_slots', 'futsals.id', '=', 'futsal_slots.futsal_id')
                      ->groupBy('futsals.id')
                      ->orderByRaw('MIN(futsal_slots.price) DESC');
                break;
        ...
        $transformedData = $futsals->map(function($futsal) use ($today, $currentTime) {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'current_page' => $futsals->currentPage(),
                'last_page' => $futsals->lastPage(),
                'per_page' => $futsals->perPage(),
                'total' => $futsals->total(),
            ]
        ]);
   ...
   /**
    ...
    public function show($id): JsonResponse
    {
        try {
            ...
            $sortedSlots = $allSlots->sortBy(function($slot) {
